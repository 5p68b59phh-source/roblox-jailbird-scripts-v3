...
local function LoadHBVHub()
    ...
    if not success then
        warn("Failed to load from GitHub: " .. tostring(err))
        print("Using built-in HBV Hub with Silent Aim & Device Spoofer...")
        
        -- Load the complete script directly with Silent Aim and Device Spoofer
        loadstring([[
                ...
                if getgenv().SilentAimSettings.Enabled and method == "FireServer" then
                    if tostring(self) == "Damage" then
                        ...
                        if target and target.Character and math.random(1, 100) <= getgenv().SilentAimSettings.HitChance then
                            ...
                            if targetPart then
                                -- Modify the hit data to target the player
